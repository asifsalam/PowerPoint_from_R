...
sort_font[["Size"]] <- 14
sort_font[["Color"]] <- pp_rgb(175,175,175)
sort_text[["Width"]] <- 110
sort_text[["Height"]] <- 15
...
bta_line[["ForeColor"]][["RGB"]] <- pp_rgb(102,255,220)

bta <- button_alpha[["TextFrame"]][["TextRange"]]
bta[["Text"]] <- "Title"
bta_font <- bta[["Font"]]
bta_font[["Size"]] <- 14
bta_font[["Color"]] <- pp_rgb(102,255,220)

button_alpha[["Width"]] <- 80
...
btd_line[["ForeColor"]][["RGB"]] <- pp_rgb(102,255,220)

btd_font <- btd[["Font"]]
btd_font[["Size"]] <- 14
btd_font[["Color"]] <- pp_rgb(102,255,220)

button_date[["Width"]] <- 80
...
for (i in 1:nrow(films)) {
    ...
    images[[as.character(i)]] <- slide1[["Shapes"]]$AddPicture(img_file,TRUE,FALSE,x+2,y+2,image_width-4,image_height-4)
    ...
    animate_image(seq_alpha,image,button_alpha,path,1.5)
    path <- paste0("M",l1,",",l2," L0,0")
    animate_image(seq_date,image,button_date,path,1.5)
...
chart_font[["Color"]] <- pp_rgb(102,255,220)
...
for (i in 1:nrow(box_office)) {
    ...
    bar <- slide1[["Shapes"]]$AddShape(ms$msoShapeRectangle,x,y,bar_w,bar_height)
    bar_fill <- bar[["Fill"]]
    ...
    line[["Style"]] <- ms$msoLineSingle
    line[["Weight"]] <- 2
    line[["ForeColor"]][["RGB"]] <- pp_rgb(0,0,0)
    ...
    bar_font[["Color"]][["RGB"]] <- pp_rgb(209,255,244)
    ...
    year_font[["Size"]] <- 9
    year_font[["Color"]][["RGB"]] <- pp_rgb(209,255,244)
    ...
    click_box_fill[["ForeColor"]][["RGB"]] <- pp_rgb(100,100,100)
    click_box_fill[["Transparency"]] <- 0.95
    
    line <- click_box[["Line"]]
    line[["ForeColor"]][["RGB"]] <- pp_rgb(100,100,100)
    line[["Visible"]] <- 0
...
line1 <- slide1[["Shapes"]]$AddLine(x,y,slide_width-5,y)
line1_attr <- line1[["Line"]]
line1_attr[["Weight"]] <- 1
line1_attr[["ForeColor"]][["RGB"]] <- pp_rgb(102,255,220)
...
label1_font[["Color"]][["RGB"]] <- pp_rgb(209,255,244)
label1_font[["Size"]] <- 14
...
line2 <- slide1[["Shapes"]]$AddLine(x,y,slide_width-5,y)
line2_attr <- line2[["Line"]]
line2_attr[["Weight"]] <- 1
line2_attr[["ForeColor"]][["RGB"]] <- pp_rgb(102,255,220)
...
label2_font[["Size"]] <- 14
label2_font[["Color"]][["RGB"]] <- pp_rgb(209,255,244)
